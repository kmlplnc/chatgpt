...
async function cleanTestClients() {
  try {
    ...
    const deleteMeasurementsResult = await client.query(
      "DELETE FROM measurements WHERE clientId IN (SELECT id FROM clients WHERE userId = 2 AND id NOT IN ($1))",
      [keepIds]
    );
    ...
    const deleteResult = await client.query(
      "DELETE FROM clients WHERE userId = 2 AND id NOT IN ($1)",
      [keepIds]
    );
